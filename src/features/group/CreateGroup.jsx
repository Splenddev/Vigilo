...
const CreateGroup = () => {
    ...
    switch (name) {
      case 'courseCode':
        if (!value) {
          ...
          errors[name] = 'Format: 2-4 letters + 3 digits (e.g., CS101, MATH201)';
        } else {
          delete errors[name];
        }
        break;
      ...
      case 'venue':
        if (!value) {
          errors[name] = 'Please specify the primary venue where this class will be held';
        } else {
          delete errors[name];
        }
        break;
  ...
  const canProceedToStep = (step) => {
    ...
    return stepFields.every(field => formData[field] && !validationErrors[field]);
  };
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">
            Set up a course-based group to manage your students, track attendance, and organize learning sessions effectively.
          </p>
        </div>
      ...
      >
            ...
            <div className="space-y-6">
              ...
              <div className="bg-blue-300/20 border border-blue-500/30 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  <LuInfo className="w-5 h-5 text-blue-500 mt-0.5" />
                  <div>
                    <h3 className="text-blue-500 font-medium mb-1">What is this step about?</h3>
                    <p className="text-blue-300 text-sm">
                      This information helps organize your group within your institution's academic structure. 
                      Students will see these details when they join your group.
                    </p>
                  </div>
                </div>
              </div>
              ...
              />
                ...
                <SelectField
                  ...
                  onChange={handleChange}
                  options={formData.faculty ? departments[formData.faculty]?.map(dept => ({ value: dept, label: dept })) || [] : []}
                  placeholder={formData.faculty ? "Select department" : "Choose faculty first"}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-purple-300/20 border border-purple-500/30 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-purple-500 font-medium mb-1">Naming your group</h3>
                    <p className="text-purple-300 text-sm">
                      A good group name should be clear and include the course code.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {/* Name Suggestions */}
              {showSuggestions && generateGroupNameSuggestions(formData).length > 0 && (
                <div className="bg-bg-secondary rounded-lg p-4 border border-bg-glass-md">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-sm font-medium text-yellow-400">Suggested Names</h3>
                  </div>
                  <p className="text-xs text-gray-400 mb-3">
                    Based on your course information. Click any suggestion to use it:
                  </p>
                  <div className="space-y-2">
                    {generateGroupNameSuggestions(formData).map((suggestion, index) => (
                      <button
                        key={index}
                        type="button"
                        onClick={() => {
            ...
            <div className="space-y-6">
              ...
              <div className="bg-green-300/20 border border-green-500/30 rounded-lg p-4 mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-green-500 font-medium mb-1">Almost done!</h3>
                    <p className="text-green-300 text-sm">
                      Review all the information below. Once created, you can modify most details later, 
                      but the course code and academic structure will be permanent.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="card space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <InfoRow label='Course Code' icon={LuBookOpen} textColor='text-blue-400' subLabel={formData.courseCode} />
                    <InfoRow label='Faculty' icon={LuBuilding} textColor='text-blue-400' subLabel={formData.faculty} />
                    <InfoRow label='Department' icon={LuGraduationCap} textColor='text-blue-400' subLabel={formData.department} />
                    <InfoRow 
                      label='Venue' 
                      icon={LuMapPin} 
                  ...
                  <div className="space-y-4">
                    <InfoRow label='Academic Level' icon={LuUsers} textColor='text-purple-400' subLabel={`${formData.level}L`} />
                    <InfoRow label='Group Name' icon={LuFileText} textColor='text-purple-400' subLabel={formData.name} />
                    <InfoRow label='Created By' icon={LuUser} textColor='text-purple-400' subLabel={`${user.firstName} ${user.lastName}`} />
                  </div>
                </div>
            ...
            <div className="flex gap-3">
              {currentStep < steps.length ? (
                ...
                <Button
                  ...
                  className="px-8 py-3 bg-green-600 hover:bg-green-500 disabled:bg-green-800 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center gap-2 font-medium" loading={loading}
                >
        ...
        <div className="mt-8 card">
          ...
          <div className="space-y-2 text-sm text-t-secondary">
            <p>• Students can search for and join your group using the course code</p>
            <p>• You'll get a unique group ID that you can share directly with students</p>
            <p>• You can start creating sessions, taking attendance, and sharing resources</p>
            <p>• Group settings can be modified anytime from the group management page</p>
          </div>
        </div>
      </div>
    </div>
